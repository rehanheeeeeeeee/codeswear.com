...
export default function CartProduct({ item, bg }) {
  ...
  const { quantity, price, title, category, img, size, color } = item;
  return (
    <div className={styles.container(bg)}>
      <div className="flex items-center space-x-2">
        ...
        <div className="">
          ...
          <p className="text-xs mb-1 text-gray-500 font-md capitalize">
            {title} ({size} / {color})
          </p>
          <p className="text-xs">
      ...
      <div className="mx-1 space-x-2 flex items-center">
        <span
          onClick={() =>
            dispatch(
              RemoveFromCart({ title, category, size, color, img, price })
            )
          }
        ...
        <span
          onClick={() => {
            dispatch(addToCart({ title, category, size, color, img, price }));
          }}
          className={styles.quantity}
        >
          <AiFillPlusCircle />
