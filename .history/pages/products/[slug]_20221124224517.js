...
export const getServerSideProps = async (context) => {
  ...
  items.forEach((item) => {
    if (item.color in types) {
      types[item.color].push(item.size);
    } else {
...
export default function ProductPage({ item, types }) {
  const { title, img, category, desc, price, slug, _id } = item;
  const router = useRouter();
  const [pin, setPin] = useState("");
  const colors = router.query.colors.split(",");
  const [selectedSize, setSelectedSize] = useState(Object.values(types)[0][0]);
  ...
  useEffect(() => {
    setSelectedSize(Object.values(types)[0][0]);
  }, [types]);
    ...
    <section className="text-gray-600 body-font overflow-hidden">
      ...
      <div className="container px-5 py-24 mx-auto">
        <div className="lg:w-4/5 mx-auto flex flex-wrap bg-white">
          <Image
            alt="ecommerce"
            className="lg:w-1/2 bg-white w-full lg:h-96 h-64 object-contain bg-white rounded"
            src={img}
            width={1920}
            height={256}
          />
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              ...
              <div className="flex ml-6 items-center">
                ...
                <div className="relative">
                  ...
                  >
                    {Object.values(types)[0].map((size, index) => (
