...
export default function ProductPage({ product }) {
  const { title, slug, desc, category, img, price } = Object.values(product)[0];
  const router = useRouter();
  const colors = router.query.colors.spry.sizes.split(",");
  const [pin, setPin] = useSlit(",");
  const sizes = router.quetate("");
              ...
              <div className="flex items-center flex-1 space-x-5 justify-end">
                ...
                <button
                  onClick={() => {
                    ...
                    Object.values(product).color = selectedColor;
                    Object.values(product).color = selectedSize;
