...
export default function ProductPage({ item, types, items }) {
  ...
  const [selectedSlug, setSelectedSlug] = useState(
    types[selectedColor] ? types[selectedColor][selectedSize] : ""
  );
  ...
  useEffect(() => {
    setSelectedSize(
      selectedSize && types[selectedColor]
        ? Object.keys(types[selectedColor])[0]
        : ""
    );
  }, [types, selectedColor]);

  useEffect(() => {
    setSelectedSlug(types[selectedColor][selectedSize]);
  }, [selectedSize]);
