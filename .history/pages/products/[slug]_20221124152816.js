...
export const getServerSideProps = async (context) => {
  const { title, size } = context.query;
  if (!mongoose.connections[0].readyState) {
    mongoose.connect(process.env.NEXT_PUBLIC_MONGO_ID);
  }
  let product = {};
  let items = await Product.find({ title: title, size: size });
  items.forEach((item) => {
    if (item.title in product) {
      product[item.title].sizes.push(product[item.size]);
    } else {
      product[item.title] = item;
      product[item.title].sizes = [product[item.title].size];
      delete product[item.title].size;
    }
  });
