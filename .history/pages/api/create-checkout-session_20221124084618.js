...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    const { basket, email, city, state, pinCode, address, phone } = req.body;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        email,
        productIds: JSON.stringify(basket.map((item) => item.id)),
        address: JSON.stringify({ city, pinCode, address, state }),
        phone,
      },
    });
