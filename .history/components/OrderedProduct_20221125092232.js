...
export default function OrderedProduct({ item, setImage }) {
  const { img, quantity, title, price, size, color } = item;
  return (
    <div
      onClick={() => setImage(img)}
      className="flex border-t cursor-pointer border-gray-200 py-2 items-center px-2 hover:bg-gray-100 transition duration-100 ease-in rounded-sm"
    >
