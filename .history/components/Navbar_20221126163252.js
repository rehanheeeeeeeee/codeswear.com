...
const styles = {
  container:
    "sticky md:fixed left-0 right-0 top-0 z-10 bg-white w-full flex items-center md:space-x-5 flex-col md:px-8 pb-3 pt-1 md:justify-start shadow-md md:flex-row  space-y-3 justify-between",
  ...
  dropDown:
    "absolute flex flex-col md:top-5 md:right-20 top-[106px] bg-pink-300 rounded-lg shadow-lg p-3 space-y-2",
  userOption:
    "px-2 transition cursor-pointer ease-in duration-100 rounded-lg  hover:bg-pink-400 ",
};
          ...
          <>
            ...
            {dropdown && (
              <motion.div
                ...
                onMouseLeave={() => setDropdown(false)}
              >
                <Link href={"/account"} className={styles.userOption}>
                  My Account
                </Link>
                <Link href={"/orders"} className={styles.userOption}>
                  Orders
                </Link>
