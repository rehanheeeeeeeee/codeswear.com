...
export default function CartProduct({ item, bg }) {
  ...
  const { id, quantity, price, title, category, img, size, color } = item;
  return (
    <div className={styles.container(bg)}>
      ...
      <div className="mx-1 space-x-2 flex items-center">
        <span
          onClick={() =>
            dispatch(
              RemoveFromCart({ id, title, category, size, color, img, price })
            )
          }
        ...
        <span
          onClick={() => {
            dispatch(
              addToCart({ id, title, category, size, color, img, price })
            );
          }}
          className={styles.quantity}
        >
